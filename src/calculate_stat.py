#!/usr/bin/env python3
"""
Distributed latent mean/var estimation for a pre-trained stage-1 model.

- Pure PyTorch DDP (nccl), NO torchxla.
- Uses LayerNormwStatistics as default for 3D/4D latents:
    (B,C)           -> BatchNorm1d(C)
...
def _extract_latents(model, images: torch.Tensor) -> torch.Tensor:
    """
    Supports both:
      - model.encode(images) -> Tensor
      - model.encode(images) -> object with .zs
    """
...
def main(args: argparse.Namespace) -> None:
    ...
    # Folder naming and SAVE_FOLDER override.
    ...
    with autocast(**autocast_kwargs):
        z0 = _extract_latents(model, images0)
    ...
    for _ in range(1):
        ...
        for images, _indices in iterator:
            ...
            with autocast(**autocast_kwargs):
                z = _extract_latents(model, images)
