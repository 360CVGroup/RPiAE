    ...
    with torch.inference_mode():
        for _, label in iterator: # don't actually need images at sampling time
            ...
            with autocast(**autocast_kwargs):
                ...
                samples = stage1_model.decode(samples).clamp(0,1)
...
def evaluate_reconstruction_distributed(
    ...
    if reference_npz_path is None:
        ...
        np.savez(ref_shard_path, arr_0=np.stack(reference_images))

    if rank == 0:
        print(f"[Rank {rank}] Saved {len(reconstructions)} reconstructions to {shard_path}")
        if reference_npz_path is None:
            print(f"[Rank {rank}] Saved {len(reference_images)} reference images to {ref_shard_path}")
    ...
    if rank == 0:
        ...
        # Load or build reference NPZ
        if reference_npz_path is None:
            all_refs = []
            for r in range(world_size):
                ref_shard_file = os.path.join(temp_dir, f"ref_{global_step:07d}_{r:02d}.npz")
                shard_data = np.load(ref_shard_file)["arr_0"]
                all_refs.append(shard_data)
            ref_images = np.concatenate(all_refs, axis=0)[:num_samples]
            reference_npz_path = os.path.join(experiment_dir, f"reference_inputs_step{global_step:07d}.npz")
            np.savez(reference_npz_path, arr_0=ref_images)
            print(f"[Eval] Built reference NPZ from current input images: {reference_npz_path}, shape: {ref_images.shape}")
        else:
