...
def _fix_adamw_fused_state(optimizer: torch.optim.Optimizer):
    # align state tensors to each param's dtype/device/layout
        ...
        for p in group["params"]:
            ...
            for k, v in st.items():
                if torch.is_tensor(v):
                    v = v.to(device=p.device, dtype=p.dtype, non_blocking=True)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train Stage-2 transport model on RPiAE latents.")
    ...
    parser.add_argument("--compile", action="store_true", help="Use torch compile (for rpiae.encode and model.forward).")
...
def main():
    ...
    if rpiae_config is None or model_config is None:
    ...
    #### Model init
    rpiae = instantiate_from_config(rpiae_config).to(device)
    rpiae.eval()
    ...
    if args.compile:
        try:
            ...
            print('RPiAE encode compile failed, falling back to no compile')
        try:
    ...
    ddp_model.train()
    # No need to put RPiAE into DDP since it is frozen.
    ...
    if maybe_resume_ckpt_path is not None:
        ...
        if ckpt_path.is_file():
            ...
            # _fix_adamw_fused_state(optimizer)
    ...
    if rank == 0:
        num_params = sum(p.numel() for p in rpiae.parameters())
        logger.info(f"Stage-1 RPiAE parameters: {num_params/1e6:.2f}M")
