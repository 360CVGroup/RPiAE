...
def main(args):
    ...
    rpiae_config, model_config, transport_config, sampler_config, guidance_config, misc, _, _ = parse_configs(cfg)
    ...
    config_name = os.path.splitext(os.path.basename(str(args.config)))[0]
    ...
    base_components = [config_name, ckpt_string_name, guidance_tag, f"bs{args.per_proc_batch_size}", args.description]
    if mode == "ODE":
    ...
    for step_idx in pbar:
        with autocast(**autocast_kwargs):
            ...
            samples = rpiae.decode(samples).clamp(0, 1)
