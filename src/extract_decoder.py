#!/usr/bin/env python3
"""
Extract stage-1 decoder weights and save to a path.
...
def _get_stage1_config(full_cfg: Any) -> Any:
    """
    Resolve the stage-1 model config.
      full_cfg = OmegaConf.load(...)
      (stage1_config, *_) = parse_configs(full_cfg)
    """
    try:
        ...
        for k in ("stage_1", "stage1", "model"):
            if k in full_cfg:
                return full_cfg[k]
        raise KeyError(
            "Could not find stage-1 config. Expected utils.train_utils.parse_configs(full_cfg) "
            "or one of: stage_1/stage1/model."
        )


def main() -> None:
    ap = argparse.ArgumentParser()
    ap.add_argument("--config", type=str, required=True, help="YAML config used to instantiate stage-1 model.")
    ...
    stage1_cfg = _get_stage1_config(full_cfg)

    # Instantiate
    model = instantiate_from_config(stage1_cfg)
