...
def is_stage2(training_stage: str) -> bool:
    return training_stage == "stage2"
...
def piv_patch_mse(teacher_feat: torch.Tensor, student_feat: torch.Tensor) -> torch.Tensor:
    ...
    Patch alignment loss:
      - teacher_feat detach
      - MSE over patch tokens
    """
    t = _feat_to_tokens(teacher_feat).detach()
    s = _feat_to_tokens(student_feat)
...
def main():
    ...
    sample_vb = training_cfg.get("sample_vb", False)
    ...
    init_ckpt = rae_config.get("init_ckpt", None)
    if init_ckpt is not None:
        ckpt = torch.load(init_ckpt, map_location="cpu")
        sd = ckpt.get("model", ckpt)

    # Load only encoder/decoder-related modules from the checkpoint.
        encdec_sd = {
            k: v
            for k, v in sd.items()
            if k.startswith("encoder.")
            or k.startswith("decoder.")
            or k.startswith("vb_feature_encoder.")
            or k.startswith("vb_feature_decoder.")
        }
        missing, unexpected = rpiae.load_state_dict(encdec_sd, strict=False)
        
        if rank == 0:
            logger.info(f"[Init] Loaded encoder/decoder from {init_ckpt}")
            logger.info(f"[Init] Missing keys (partial ok): {len(missing)}")
    ...
    # only train decoder (unchanged behavior)
    if not encoder_trainable:
        rpiae.encoder.eval()
        rpiae.encoder.requires_grad_(False)

    rpiae.decoder.train()
    if hasattr(rpiae, "vb_feature_decoder") and rpiae.vb_feature_decoder is not None:
        rpiae.vb_feature_decoder.train()
    ...
    # VB-related stage flags from model
    training_stage = getattr(ddp_model.module, "training_stage", "stage3")
    use_vb = bool(getattr(ddp_model.module, "use_vb", False))
    vb_feat_w = float(getattr(ddp_model.module, "vb_feat_loss_weight", 1.0))
    vb_kl_w = float(getattr(ddp_model.module, "vb_kl_weight", 1e-4))
    vb_rec_w = float(getattr(ddp_model.module, "vb_rec_weight", 0.05))
    if rank == 0:
        logger.info(f"[Stage] training_stage={training_stage}, use_vb={use_vb}")
        if is_stage2(training_stage):
            logger.info(f"[VB Loss Weights] feat={vb_feat_w}, kl={vb_kl_w}, rec={vb_rec_w}")

        # Optional: param count for VB modules
      
        if hasattr(ddp_model.module, "vb_feature_encoder") and ddp_model.module.vb_feature_encoder is not None:
            count_params(ddp_model.module.vb_feature_encoder, "Model.vb_feature_encoder", verbose_topk=10, logger=logger)
        if hasattr(ddp_model.module, "vb_feature_decoder") and ddp_model.module.vb_feature_decoder is not None:
            count_params(ddp_model.module.vb_feature_decoder, "Model.vb_feature_decoder", verbose_topk=10, logger=logger)
    ...
    #### Opt init
    # Optimizer params by stage:
    training_stage = getattr(ddp_model.module, "training_stage", "stage3")
    if is_stage2(training_stage):
        gen_params = []
        if hasattr(ddp_model.module, "vb_feature_encoder") and ddp_model.module.vb_feature_encoder is not None:
            gen_params += list(ddp_model.module.vb_feature_encoder.parameters())
        if hasattr(ddp_model.module, "vb_feature_decoder") and ddp_model.module.vb_feature_decoder is not None:
            gen_params += list(ddp_model.module.vb_feature_decoder.parameters())
        # filter requires_grad just in case
        gen_params = [p for p in gen_params if p.requires_grad]
        if rank == 0:
            logger.info(f"[Optimizer] Stage2 (VB): optimizing {len(gen_params)} parameter tensors")
    else:
        # decoder stage: keep old behavior
        ...
        gen_params = [p for p in gen_params if p.requires_grad]
        if rank == 0:
            logger.info(f"[Optimizer] Stage3: optimizing decoder (+encoder_trainable={encoder_trainable})")
    ...
    if rank == 0:
        ...
        logger.info(
            f"GAN training starts at epoch {gan_start_epoch}, discriminator updates start at epoch {disc_update_epoch}, "
            f"LPIPS loss starts at epoch {lpips_start_epoch}."
        )
        ...
        logger.info(f"[Stage] training_stage={getattr(ddp_model.module,'training_stage','stage3')}")
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for step, (images, _) in enumerate(loader):
            ...
            # decoder stage = original behavior
            if is_stage2(training_stage):
                use_gan = False
                train_disc = False
                use_lpips = False
            ...
            enc_last_layer = None  # for adaptive piv (decoder stage only)

            # placeholders for logging (defined in both branches)
            feat_l2 = None
            feat_kl = None

            with autocast(**autocast_kwargs):
                # =========================
                # (A) VB stage: only train VB feature encoder/decoder
                # =========================
                if is_stage2(training_stage) and use_vb:
                    # expect forward supports return_vb=True:
                    # recon, z_target, z_t, vb_posterior, vb_dec = model(images, return_vb=True)
                    recon, z_target, vb_posterior, vb_dec = ddp_model(images, return_vb=True)

                    recon_normed = recon * 2.0 - 1.0
                    # rec_loss = (recon - images).abs().mean()
                    rec_loss = F.l1_loss(recon, images)
                    feat_l2 = F.mse_loss(vb_dec, z_target)
                    # keep same style as lightning: mean over batch
                    feat_kl = vb_posterior.kl(no_sum=True)
                    ...
                    total_loss = vb_feat_w * feat_l2 + vb_kl_w * feat_kl + vb_rec_w * rec_loss

                    # dummy tensors for unified logging keys
                    lpips_loss = rec_loss.new_zeros(())
                    gan_loss = rec_loss.new_zeros(())
                    piv_loss = rec_loss.new_zeros(())
                    piv_aw = rec_loss.new_zeros(())
                    adaptive_weight = rec_loss.new_zeros(())
                    recon_base = total_loss

                # =========================
                # (B) Decoder stage: KEEP OLD BEHAVIOR
                # =========================
                else:
                    if use_teacher:
                        recon, z_s, z_t = ddp_model(images, return_detail=True)
                    else:
                        recon = ddp_model(images, sample_vb=sample_vb)
                        z_s, z_t = None, None
                    ...
                    if (piv_weight > 0.0) and (z_s is not None) and (z_t is not None):
                        ...
                        # keep backward-compatible layer selection (more robust)
                        if hasattr(ddp_model.module.encoder, "get_piv_last_layer"):
                            ...
                            # some encoders expose method piv_last_layer() or attribute; try best effort
                            try:
                                enc_last_layer = ddp_model.module.encoder.piv_last_layer()
                            except Exception:
                                enc_last_layer = next(
                                    reversed([p for p in ddp_model.module.encoder.parameters() if p.requires_grad])
                                )
                        else:
            ...
            # =========================
            # Outside autocast: adaptive weights / total loss
            # =========================
            training_stage = getattr(ddp_model.module, "training_stage", "stage3")
            if not is_stage2(training_stage):
                if (
                    (piv_weight > 0)
                    and (piv_loss is not None)
                    and adaptive_piv
                    and (global_step >= piv_warmup_steps)
                    and (enc_last_layer is not None)
                ):
                ...
                if use_gan:
                    adaptive_weight = calculate_adaptive_weight(recon_base, gan_loss, last_layer, max_d_weight)
            ...
            # =========================
            # Discriminator training (decoder stage only)
            # =========================
            ...
            if is_stage2(training_stage) and feat_l2 is not None and feat_kl is not None:
                epoch_metrics["vb_feat_l2"] += feat_l2.detach()
                epoch_metrics["vb_feat_kl"] += feat_kl.detach()
            num_batches += 1
            ...
            if log_interval > 0 and global_step % log_interval == 0 and rank == 0:
                ...
                if is_stage2(training_stage) and feat_l2 is not None and feat_kl is not None:
                    stats.update(
                        {
                            "vb/feat_l2": feat_l2.detach().item(),
                            "vb/kl": feat_kl.detach().item(),
                            "vb/rec": rec_loss.detach().item(),
                            "vb/w_feat": float(getattr(ddp_model.module, "vb_feat_loss_weight", 1.0)),
                            "vb/w_kl": float(getattr(ddp_model.module, "vb_kl_weight", 1e-4)),
                            "vb/w_rec": float(getattr(ddp_model.module, "vb_rec_weight", 0.05)),
                        }
                    )
                ...
                logger.info(
                    ...
                    + ", ".join(f"{k}: {v:.4f}" for k, v in stats.items() if isinstance(v, (float, int)))
                )
                if args.wandb:
                    wandb_utils.log(stats, step=global_step)

            # sampling: use forward to respect stage behavior
            if global_step % sample_every == 0 and rank == 0:
                logger.info("Generating EMA samples...")
                with torch.no_grad():
                    sample_images = images[:4]
                    # IMPORTANT: use forward instead of decode(encode)
            ...
            if do_eval and (eval_interval > 0 and global_step % eval_interval == 0 and global_step != 0 ):
                ...
                for eval_mod, mod_name in eval_models:
                    ...
                    eval_stats = (
                        {f"eval_{mod_name}/{k}": v for k, v in eval_stats.items()} if eval_stats is not None else {}
                    )
                    if args.wandb:
        ...
        if rank == 0 and num_batches > 0:
            ...
            if epoch_metrics.get("vb_feat_l2", None) is not None and epoch_metrics.get("vb_feat_kl", None) is not None:
                # only meaningful when training_stage == stage2, but harmless otherwise
                epoch_stats.update(
                    {
                        "epoch/vb_feat_l2": (epoch_metrics["vb_feat_l2"] / num_batches).item(),
                        "epoch/vb_feat_kl": (epoch_metrics["vb_feat_kl"] / num_batches).item(),
                    }
                )

            if "disc_loss" in epoch_metrics and epoch_metrics["disc_loss"].numel() > 0:
                # only set when disc_metrics existed at least once
                epoch_stats.update(
                    {
