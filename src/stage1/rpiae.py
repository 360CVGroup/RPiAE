...
class RPiAE(nn.Module):
    ...
    def encode(self, x: torch.Tensor, return_detail=False):
        # normalize input
        _, _, h, w = x.shape
        ...
        if self.training and self.noise_tau > 0:
            z_noise = self.noising(z)
        else:
            z_noise = z
        if self.reshape_to_2d:
            b, n, c = z_noise.shape
            h = w = int(sqrt(n))
            z_noise = z_noise.transpose(1, 2).view(b, c, h, w)
            b, n, c = z.shape
            h = w = int(sqrt(n))
            z = z.transpose(1, 2).view(b, c, h, w)
        if self.do_normalization:
            latent_mean = self.latent_mean.to(z_noise.device) if self.latent_mean is not None else 0
            latent_var = self.latent_var.to(z_noise.device) if self.latent_var is not None else 1
            z_noise = (z_noise - latent_mean) / torch.sqrt(latent_var + self.eps)
        if self.use_teacher:
            z_t = self.teacher_encoder(x)
            if self.reshape_to_2d:
                b, n, c = z_t.shape
                h = w = int(sqrt(n))
                z_t = z_t.transpose(1, 2).view(b, c, h, w)
        else:
            z_t = None
        if return_detail:
            return z_noise, z, z_t
        return z_noise
    ...
    def forward(self, x: torch.Tensor, return_detail=False) -> torch.Tensor:
        if return_detail:
            z, z_s, z_t = self.encode(x, return_detail=True)
        else:
            z = self.encode(x)
        x_rec = self.decode(z)
        if return_detail:
            return x_rec, z_s, z_t
        return x_rec
